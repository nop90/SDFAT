...
uint32_t sdmmc_cid[4];
volatile uint32_t sdmmc_cardready = 0;

static int sdmmc_timeout = 0;
static int sdmmc_gotcmd8reply = 0;
static int sdmmc_sdhc = 1;

//Made Device Independent by Xerpi :)
void swiDelay(uint32_t loops){
...
//---------------------------------------------------------------------------------
int sdmmc_send_command(uint16_t cmd, uint16_t arg0, uint16_t arg1) {
//---------------------------------------------------------------------------------
	//For status deltas
	uint16_t is_stat0, was_stat0;
	uint16_t is_stat1, was_stat1;
...
//---------------------------------------------------------------------------------
	...
	sdmmc_cid[0] = resp0 | (uint32_t)(resp1<<16);
	sdmmc_cid[1] = resp2 | (uint32_t)(resp3<<16);
	sdmmc_cid[2] = resp4 | (uint32_t)(resp5<<16);
	sdmmc_cid[3] = resp6 | (uint32_t)(resp7<<16);
...
//---------------------------------------------------------------------------------
int sdmmc_sdcard_readsector(unsigned long sector_no, void *out) {
        uint8_t *out8 = (uint8_t*)out;
        uint16_t resp0, resp1, tmp;
        int i;
...
//---------------------------------------------------------------------------------
int sdmmc_sdcard_readsectors(uint32_t sector_no, int numsectors, void *out) {
//---------------------------------------------------------------------------------
        uint8_t *out8 = (uint8_t*)out;
        uint16_t resp0, resp1, tmp;
        int i;
...
//---------------------------------------------------------------------------------
int sdmmc_sdcard_writesector(uint32_t sector_no, void *in) {
//---------------------------------------------------------------------------------
...
//---------------------------------------------------------------------------------
int sdmmc_sdcard_writesectors(uint32_t sector_no, int numsectors, void *in) {
//---------------------------------------------------------------------------------
